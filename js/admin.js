...
async function loadPayrollSummary() {
  ...
  const [allStaff, records, specialDays, taxKou] = await Promise.all([
    DB.getStaff(),
  ...
  activeStaff.forEach(staff => {
    ...
    if (staff.type === 'hourly') {
      staffRecords.forEach(r => {
        ...
        totalMins += mins;
        if (r.is_special_day) specialCount++;
    ...
    <div class="payslip">
      ...
      <div class="table-scroll">
        <table class="data-table">
          <thead>
            <tr><th>日付</th><th>出勤(実)</th><th>退勤(実)</th><th>出勤(計)</th><th>退勤(計)</th><th>労働時間</th><th>時給</th><th>日給</th></tr>
          </thead>
