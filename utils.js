...
function roundUpClockIn(timeStr) {
  ...
  const r=mins%30; if(r===0) return timeStr;
...
function calcWorkMinutes(clockIn, clockOut) {
  ...
  const diff=timeToMinutes(clockOut)-timeToMinutes(clockIn);
...
function staffTypeLabel(type) {
  const map={hourly:'時給スタッフ',senzoku:'専従者',employee:'社員',officer:'役員'};
...
function calcTax(monthlyIncome, taxRows) {
  ...
  for (const row of sorted) { if(monthlyIncome>=row.income_from) return row.tax_amount; }
  return 0;
}
