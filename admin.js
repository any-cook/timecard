...
async function loadStaffTab() {
  var results = await Promise.all([DB.getInsuranceTable('pension'), DB.getInsuranceTable('health')]);
  _pensionTable = results[0];
  _healthTable = results[1];
  var staff = await DB.getStaff();
  var tbody = document.getElementById('staffTableBody');
  tbody.innerHTML = '';
  if (!staff.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="empty-cell">スタッフが登録されていません</td></tr>';
    return;
  }
...
async function buildGradeSelects() {
  ...
  _healthTable.slice().sort(function(a, b) { return a.grade - b.grade; }).forEach(function(r) {
...
function updateInsurancePreview() {
  ...
  var healthAmt = getInsuranceAmountByGrade(healthId, _healthTable);
...
function autoSelectGradeByMonthly() {
  ...
  var health = findInsuranceGradeByMonthly(monthly, _healthTable);
...
async function loadAttendanceRecords() {
  ...
  records.forEach(function(r) {
    ...
    staffSummary[r.staff_id].mins += workMins;
    staffSummary[r.staff_id].wage += dailyWage;
  ...
  var summaryBody = document.getElementById('staffSummaryBody');
...
async function previewCsv() {
  ...
  for (var i = 0; i < dataLines.length; i++) {
    ...
    tbody.appendChild(tr);
    if (isValid) csvParsedData.push({ staff_id: matchedStaff.id, date: date, clock_in_actual: clockIn,
      clock_out_actual: clockOut || null, clock_in_calc: roundUpClockIn(clockIn),
...
async function loadSpecialTab() {
  ...
  var tbody = document.getElementById('specialTableBody');
  tbody.innerHTML = '';
...
async function loadPayrollSummary() {
  ...
  var results = await Promise.all([DB.getStaff(), DB.getAttendance({ year: year, month: month }),
    DB.getTaxTable('kou'), DB.getInsuranceTable('pension'), DB.getInsuranceTable('health')]);
  var allStaff = results[0], records = results[1], taxKou = results[2], pensionTable = results[3], healthTable = results[4];
  var tbody = document.getElementById('payrollTableBody');
  tbody.innerHTML = ''; var grandTotal = 0;
...
async function showPayslip(staffId, year, month) {
  ...
  var healthRow = healthTable.find(function(r) { return r.id === staff.health_grade_id; });
...
async function loadTaxTable(type) {
  ...
  var tbody = document.getElementById('taxTableBody');
  tbody.innerHTML = '';
...
async function loadInsuranceTable(type) {
  ...
  var tbody = document.getElementById('insuranceTableBody');
  tbody.innerHTML = '';
...
async function previewTaxCsv() {
  ...
  var lines = text.split('\n').filter(function(l) { return l.trim(); });
  taxCsvParsed = [];
  var tbody = document.getElementById('taxCsvPreviewBody');
  tbody.innerHTML = '';
  ...
  for (var i = 0; i < dataLines.length; i++) {
    ...
    var income_from = parseInt(cols[0]);
    var tax_amount = parseInt(cols[1]);
...
async function previewInsuranceCsv() {
  ...
  var lines = text.split('\n').filter(function(l) { return l.trim(); });
  insuranceCsvParsed = [];
  var tbody = document.getElementById('insuranceCsvPreviewBody');
  tbody.innerHTML = '';
  var first = lines[0].split(',')[0];
...
async function importInsuranceCsv() {
  ...
  var label = type === 'pension' ? '厚生年金' : '健康保険';
...
async function saveTaxRow() {
  ...
  var row = { income_from: income_from, tax_amount: tax_amount };
  if (id) row.id = id;
...
async function openLeaveModal() {
  var staffId = document.getElementById('leaveStaffSelect').value;
  var staff = await DB.getStaff();
  var sel = document.getElementById('leaveModalStaff');
  sel.innerHTML = '';
  staff.filter(function(s) { return s.is_active; }).forEach(function(s) {
    var opt = document.createElement('option'); opt.value = s.id; opt.textContent = s.name;
