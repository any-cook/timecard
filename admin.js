...
// 40歳〜64歳 → 介護保険第2号被保険者 → 健康保険料に介護保険加算
...
async function loadStaffTab() {
  var results = await Promise.all([
    DB.getInsuranceTable('pension'),
    DB.getInsuranceTable('health'),
    DB.getInsuranceTable('health_nursing')
  ]);
  ...
  staff.forEach(function(s) {
    ...
    var nursing = s.birthdate ? (isNursingCare(s.birthdate) ? '<span class="badge" style="background:#fef3c7;color:#92400e;border:1px solid #d97706;">介護2号（40-64歳）</span>' : '<span class="badge badge-inactive">非該当</span>') : '-';
...
async function buildGradeSelects(tableType) {
  ...
  var labelSuffix = tableType === 'health_nursing' ? '（介護込み・40〜64歳）' : '（介護なし）';
  ...
  document.getElementById('_currentHealthTableType') ? 0 : 0;
...
function updateNursingCareStatus() {
  ...
  if (!birthdate) {
    statusEl.textContent = '生年月日を入力すると自動判定';
    statusEl.style.background = '#f1f5f9'; statusEl.style.color = 'var(--text-muted)';
    return;
  }
  var age = calcAge(birthdate);
  ...
  // 社会保険加入中なら健康保険等級セレクトを切り替え
  if (document.getElementById('staffSocialInsurance').checked) {
    var tableType = nursing ? 'health_nursing' : 'health';
    buildGradeSelects(tableType);
  }
}
...
async function saveStaff() {
  ...
  record.health_table_type = tableType;
...
async function openAttendanceAddModal() {
  ...
  var staff = await DB.getStaff();
  var sel = document.getElementById('attendanceStaff'); sel.innerHTML = '';
...
async function loadPayrollSummary() {
  ...
  var tbody = document.getElementById('payrollTableBody'); tbody.innerHTML = '';
  var grandTotal = 0;
  allStaff.filter(function(s) { return s.is_active; }).forEach(function(staff) {
    ...
    var nursingMark = (staff.health_table_type === 'health_nursing') ? ' 🏥' : '';
    var tr = document.createElement('tr');
    tr.innerHTML = '<td>' + staff.name + nursingMark + '</td>' +
...
async function importInsuranceCsv() {
  ...
  var labels = { pension: '厚生年金', health: '健康保険（介護なし）', health_nursing: '健康保険（介護込み）' };
  if (!confirmAction(insuranceCsvParsed.length + '等級で' + labels[type] + '料額表を上書きしますか？')) return;
  await DB.replaceInsuranceTable(type, insuranceCsvParsed.map(function(r) { return Object.assign({}, r, { id: _uid() }); }));
  closeModal('insuranceCsvModal'); showToast(labels[type] + '料額表を更新しました'); loadInsuranceTable(type);
