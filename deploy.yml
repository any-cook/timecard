// demo-data.js - デモ用サンプルデータ初期投入
// このファイルはデモ確認用です。本番環境では不要です。
...
function initDemoData() {
  ...
  const attendanceData = [
    ...
    makeRecord('demo-staff-1',  4, '11:00', '19:05', false, 1050),
    makeRecord('demo-staff-1',  5, '10:15', '17:50', true,  1050), // 金曜
    makeRecord('demo-staff-1',  6, '10:00', '18:00', true,  1050), // 土曜
    ...
    makeRecord('demo-staff-2',  7, '09:00', null,    true,  1100),  // 退勤忘れ
  ];

  localStorage.setItem('staff', JSON.stringify(staffData));
  localStorage.setItem('attendance', JSON.stringify(attendanceData));
  localStorage.setItem('special_days', JSON.stringify([
    { id: 'demo-sp-1', date: `${y}-12-31`, reason: '年末営業', created_at: new Date().toISOString() },
    { id: 'demo-sp-2', date: `${y}-01-02`, reason: '年始営業', created_at: new Date().toISOString() },
  ]));

  console.log('✅ デモデータを初期投入しました');
}
